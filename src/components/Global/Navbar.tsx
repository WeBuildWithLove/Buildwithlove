...
function Navbar() {
  ...
  return (
    <div className="flex items-center justify-between p-0 lg:p-[20px_30px] border font-manrope sticky bg-white top-0 z-20 shadow">
      ...
      <div className={`${open ? "left-0" : "left-[-100%]"} lg:hidden fixed top-0 h-screen bg-white w-full p-[20px] transition-all`}>
        <div className="flex w-full items-center justify-end">
          <FiPlus onClick={() => setOpen(!open)} className="text-[30px] rotate-45" />
        </div>
        <div className="flex flex-col items-start space-y-[20px]">
          {links.map((link, index) => (
            <Link
              ...
              href={link.url}
              className="text-typography2 hover:text-primary uppercase  text-[20px] font-[600] text-center"
            >
              {link.name}
            </Link>
          ))}
